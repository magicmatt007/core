...
class ModbusInstrument:
    ...
    def __init__(self, slaveAddress):
        self.instrument = minimalmodbus.Instrument(
            ModbusInstrument.port, slaveAddress
        ...
        if True:  # Configure Modbus bus parameters, if this is the first device.
            ...
            self.instrument.serial.timeout = (
                ModbusInstrument.timeout
            )  # 0.05          # seconds
            self.instrument.mode = ModbusInstrument.mode  # rtu or ascii mode
            self.instrument.clear_buffers_before_each_transaction = (
                ModbusInstrument.clear_buffers_before_each_transaction
            )
    ...
    def factoryDate(self):
        ...
        byte2 = (
            value2 // 0x100
        )  # divide with full integer  #divmod(value1, 0x100) = higher byte
        print([value1, value2])
        print(byte1, byte2)
        year = 2000 + value1
        month = byte2
    ...
    def monitorPositionChange(self):
        ...
        for i in range(0, int(runtime_max / interval)):
            ...
            if current == previous:
                ...
                counter = 0
            print(
                f"{self.instrument.address}: Current ! position: {current}, ({(current/10000*90):.1f}°)"
            )
            i = +1
            time.sleep(interval)
